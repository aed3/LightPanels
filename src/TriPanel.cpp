...
TriPanel::TriPanel(int pin, uint16_t numLeds, CornerLocation local,
  ...
  PanelSegment segment2(
    segment2Location, corner1 + 1, corner2, *this);
  PanelSegment segment3(
    segment3Location, corner2 + 1, numLeds - 1, *this);
...
void TriPanel::breath(
  ...
  fadeIn(maxBrightness, constantColor, fadeSpeed);

  runFunctionLater([this, fadeSpeed,
                     constantColor]() { fadeOut(0, constantColor, fadeSpeed); },
    fadeSpeed);
  runFunctionLater([this, color, maxBrightness,
                     fadeSpeed]() { breath(maxBrightness, fadeSpeed, color); },
    fadeSpeed * 2);
}
...
void TriPanel::fadeIn(
  ...
  for (int b = currentBrightness; b <= maxBrightness; b++) {
...
void TriPanel::rainbow(double loops, uint8_t speed) {
  ...
  const int colorsToSkip = 573 / pixelCount;

  for (int i = 0; i < loops * pixelCount; i++) {
    auto fn = [this, i, pixelCount, colorsToSkip]() {
      for (int n = 0; n < pixelCount; n++) {
        setPixelColor(n, rainbowColors[((i + n) * colorsToSkip) % 573]);
      }
    };

    runFunctionLater(fn, i * functionDelay);
  }
}
...
void TriPanel::colorSpin(double loops, uint8_t speed) {
  ...
  for (int currentPixel = 0; currentPixel < pixelCount * loops;
       currentPixel++) {
...
void TriPanel::clearFunctions() { functionSequence.clear(); }

void TriPanel::begin() {
#ifdef ARDUINO
  lights.begin();
#else
  lights.begin(overallLocation);
#endif
  lights.clear();
  setBrightness(128);
  show();
